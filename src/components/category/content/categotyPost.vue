    ...
    <div class="col-md-12">
      <div v-for="post in categoryItemList" v-bind:key="post.slug"  class="post-entry-horzontal">
        <a v-bind:href="page + post.slug" style="width:100%;" >
          ...
          <span class="text">
            <div class="post-meta">
              <span  v-if="post.metadata.author.metadata && post.metadata.author.metadata.authorimg"  class="author mr-2"><img  v-bind:src="post.metadata.author.metadata.authorimg.imgix_url"  > {{post.metadata.author.title}}</span>&bullet;
